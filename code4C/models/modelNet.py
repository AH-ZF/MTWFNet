...
class modelNet(nn.Module):
    def __init__(self, inputfeatues, otherP=None):
        ...
        self.class_num = 4  # That is, 4-class classification
        ...
        feavecl = 128
        patchsize = 4
        patchl = feavecl // patchsize
        self.vitmodel = vitNet.ViT(vec_size=feavecl,
    ...
    def forward(self, x1, x2, x3, x4, x5, x6, labels, othersParms=None):
        ...
        x = self.chosefusion4(festures=[x, x - self.beta * x5], targets=labels)

        # VIT-Based Ablation Experiments
        rgbx = torch.unsqueeze(x, dim=1)
        ecgx = torch.unsqueeze(x6, dim=1)
        vitx = torch.cat([rgbx,  ecgx], dim=1)
        vitfusx, _ = self.vitmodel(vitx)
        vitfusx = torch.squeeze(self.con4d(vitfusx))
        x = vitfusx

        # Tangent-weighted non-modal equivalence
        coeff = math.tan(self.gamma * math.pi / 2.0)

        # Linear Ablation Experiment
        # lw=4.8
        x = x + coeff * x6


        # Final Classifier of the Model
        x = self.fc3(x)
